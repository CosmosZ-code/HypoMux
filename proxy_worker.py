...
# 格式: (网络前缀 packed, 子网掩码 packed) — 掩码位为 1 表示需匹配的网络位。
_PRIVATE_RANGES = (
    (0x0A000000, 0xFF000000),   # 10.0.0.0/8
    (0xAC100000, 0xFFF00000),   # 172.16.0.0/12
    (0xC0A80000, 0xFFFF0000),   # 192.168.0.0/16
    (0xA9FE0000, 0xFFFF0000),   # 169.254.0.0/16 (APIPA)
)


def _is_private_ipv4(ip: str) -> bool:
    """判断 IP 是否为内网/私有地址（跨子网失败不应视为网卡故障）。"""
    try:
        parts = ip.split(".")
        if len(parts) != 4:
            return False
        ...
        return False
    for start, mask in _PRIVATE_RANGES:
        if (packed & mask) == start:
            return True
    return False
...
# 辅助：判断是否启用优先级模式
# ---------------------------------------------------------------------------
def _has_priority_config(nics: List[Dict]) -> bool:
    """任意网卡携带非默认 priority 即启用优先级调度。"""
    for nic in nics:
        p = nic.get("priority", 1)
        try:
            if int(p) != 1:
                return True
...
class ProxyWorker(QThread):
    ...
    def __init__(
        ...
        self.balancer = (
            PriorityBalancer(self._selected_nics)
            if _has_priority_config(self._selected_nics)
            else RoundRobinBalancer(self._selected_nics)
        )
        ...
        try:
            ...
            try:
                ...
                if nic is None:
                    ...
                    # 跨子网私有 IP / APIPA 失败不扣健康分：不是网卡故障，
                    # 而是目标本身只存在于另一张网的子网内。
                    if not _is_private_ipv4(dst_addr):
...
class MultiPortProxyWorker(QThread):
    ...
    def __init__(
        ...
        # 每个端口一个独立 balancer（支持优先级模式）
        def _make_balancer(nics):
            return (
                PriorityBalancer(nics)
                if _has_priority_config(nics)
                else RoundRobinBalancer(nics)
            )
        self.bal_ethernet = _make_balancer(self._wired)
        self.bal_wifi = _make_balancer(self._wifi)
        self.bal_aggregation = _make_balancer(self._selected_nics)
